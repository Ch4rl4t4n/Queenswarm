...
from app.domain.agents.bees.generic import GenericBee
from app.domain.agents.bees.role_bees import (
    BlogWriterBee,
    EvaluatorBee,
    LearnerBee,
    MarketerBee,
...
from app.core.cost_governor import CostGovernor
...
from app.application.services.hive_tier import FIXED_ORCHESTRATOR_AGENT_NAME, is_fixed_orchestrator_agent
...
def specialist_roles_for_manager_lane(manager_slug: str) -> tuple[AgentRole, ...]:
    """Return suggested specialist roles for a Ballroom manager persona (sub-swarm 2–5)."""

    slug = manager_slug.strip().lower()
    return get_manager_template(slug).sub_swarm_roles


__all__ = ["instantiate_agent", "register_specialist", "specialist_roles_for_manager_lane"]
