...
from app.core.chroma_client import RECIPE_LIBRARY_COLLECTION, delete_documents_by_ids, embed_and_store
...
async def upsert_recipe_into_chroma_library(recipe: Recipe) -> str:
    ...
    if prior:
        try:
            await delete_documents_by_ids(RECIPE_LIBRARY_COLLECTION, [prior])
...
async def delete_recipe_embedding_from_chroma(*, embedding_id: str | None) -> None:
    ...
    try:
        await delete_documents_by_ids(RECIPE_LIBRARY_COLLECTION, [cleaned])
    except Exception as exc:
        ctx.warning(
