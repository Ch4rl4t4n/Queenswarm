...
function DagNode({ step }: { step: WorkflowDagStep }) {
  const tone = DAG_TONE_HEX[step.hex_tone];
  const common = (
    props: Omit<Partial<ComponentProps<typeof HexNumberBadge>>, "value">,
  ) => (
    <div title={step.description_excerpt} className="inline-flex shrink-0">
      <HexNumberBadge value={step.step_order} strokeColor={tone} sizePx={56} {...props} />
    </div>
  );

  if (step.dag_state === "failed") {
    return common({ failed: true });
  }

  if (step.dag_state === "upcoming") {
    return common({ glowColor: tone });
  }

  if (step.dag_state === "active") {
    return common({
      glowColor: tone,
      variant: "solid",
      activePulse: true,
...
export function WorkflowsSection() {
  ...
  const onPauseFeatured = async () => {
    ...
    try {
      ...
      const msg = e instanceof HiveApiError ? e.message : e instanceof Error ? e.message : "Pause failed";
      toast.error(msg);
    } finally {
      setBusyId(null);
    }
  ...
  const onCancelFeatured = async () => {
    ...
    try {
      ...
      const msg = e instanceof HiveApiError ? e.message : e instanceof Error ? e.message : "Cancel failed";
      toast.error(msg);
    } finally {
      setBusyId(null);
    }
