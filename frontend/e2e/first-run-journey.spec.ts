...
test.describe("First-run journey — Track Q UX9", () => {
  ...
  for (const viewport of VIEWPORTS) {
    test(`${viewport.name} · Mission Home shows setup Process Rail`, async ({ page, context, baseURL }) => {
      ...
      // Personal OS lite first-run: daily-start guide is the focal point; telemetry (rapid loop)
      // lives under "Zobraziť pokročilé". Setup steps (LLM keys) stay visible during first-run.
      await expect(page.getByTestId("mission-home-daily-start")).toBeVisible({ timeout: 20_000 });
