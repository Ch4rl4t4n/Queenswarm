...
const SOCIAL_PUBLISH_SNAPSHOT = {
  ...
  ready_items: [],
  audit: { enabled: true, count: 1, entries: [{ at: new Date().toISOString(), kind: "social_simulate", message: "OK", title: "Launch", channel: "instagram", mode: "simulate", ok: true }] },
  trusted_auto: { global_enabled: false, tenant_enabled: false, min_simulates_required: 5, live_enabled: false, channels: [] },
  rate_limit: { enabled: true, fail_closed: true, window_hours: 24, global_used: 0, global_max: 30, global_remaining: 30, redis_ok: true, channels: [] },
  links: { publish_queue: "/integrations?tab=studio#publish-queue" },
};
...
test.describe("Publish lane panels", () => {
  test.beforeEach(async ({ context, baseURL, page }) => {
    ...
    await page.route("**/api/proxy/execution-studio/overview", async (route) => {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ enabled: true, policy: {}, stats: {}, connections: [], packs: [], setup_steps: [] }),
      });
    });
  ...
  test("Execution Studio shows publish queue media preview and social publish rate limits", async ({ page }) => {
