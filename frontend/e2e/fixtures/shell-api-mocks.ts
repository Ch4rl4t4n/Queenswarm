...
export async function installShellApiMocks(page: Page): Promise<void> {
  await page.route("**/api/proxy/**", async (route) => {
    ...
    if (path.startsWith("dashboard/factory-launch/gumroad-publish")) {
      await route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({
          ...
          published_count: 1,
          skipped_count: 0,
          message: "Published 1 Gumroad listing(s).",
          publishes: [
            {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          autopilot_strip: {
            ...
            digest_pending: 1,
            cron_lane_count: 3,
            message: "1 digest(s) waiting — review in Digest Inbox.",
            lanes: [
              {
                id: "life_os",
                label: "Life OS",
          ...
          jarvis_advisor_strip: {
            ...
            steps: [
              {
                order: 1,
                title: "Review 1 digest(s)",
                detail: "Four Lanes produced background intel.",
                href: "/cockpit#four-lanes",
                kind: "verify",
              },
              {
                order: 2,
                title: "Calendar: Morning standup",
                detail: "Starts soon — block deep work before this meeting.",
                href: "/integrations?tab=connectors",
                kind: "calendar",
              },
              {
            ...
            analytics_href: "/apps-tools/analytics",
            research_href: "/knowledge#research-bee",
            loops_href: "/settings/harness#harness-closed-loop-presets",
          },
          agent_quality_strip: {
            enabled: true,
            status: "healthy",
            pass_rate_7d_pct: 82.5,
            pass_rate_trend: "stable",
            stuck_sessions: 0,
            active_sessions: 1,
            operator_hint: "Simulation pass rate healthy — harness verify-first loop is holding.",
            harness_href: "/settings/harness#harness-closed-review-loop",
            scorecard_href: "/cockpit#business",
          },
          jarvis_weekly_reflection_strip: {
            enabled: true,
            headline: "This week · reflection",
            message: "1 Ballroom post-mortem(s) · 2 episodic capture(s) · 3 completed session(s) — review patterns in Hive Mind.",
            week_label: "May 30 – Jun 05",
            ballroom_post_mortems_7d: 1,
            episodic_captures_7d: 2,
            sessions_completed_7d: 3,
            learning_logs_7d: 1,
            highlights: [
              {
                source: "ballroom",
                title: "Ballroom post-mortem abc",
                excerpt: "Highlights: shipped verify loop. Risks: LLM latency.",
                href: "/knowledge#hivemind",
              },
              {
                source: "episodic",
                title: "Research brief",
                excerpt: "Verified weak-signal digest with simulate-first gate.",
                href: "/knowledge#memory",
              },
            ],
            hive_mind_href: "/knowledge#hivemind",
            episodic_href: "/knowledge#memory",
            ballroom_href: "/ballroom",
          },
          weekly_compound_strip: {
            enabled: true,
            headline: "Weekly compound · gardener",
            message: "1 compound draft(s) to review · 2 Brain Pack gap(s) — approve before Hive Mind apply.",
            week_label: "2026 · week 23",
            pending_drafts: 1,
            brain_pack_gap_count: 2,
            last_run_at: null,
            hive_mind_href: "/knowledge#hivemind",
          ...
          second_brain_strip: {
            ...
            message: "1 capture(s) awaiting approve · weekly MOC + connection-intelligence tick — capture → approve → cited recall.",
            pending_captures: 1,
            connection_intelligence_weekly: true,
            wiki_href: "/knowledge?tab=wiki",
            captures_href: "/knowledge?tab=wiki#second-brain-capture-approve",
            closed_loop_href: "/settings/harness#harness-closed-loop-presets",
          },
          agent_loop_strip: {
            enabled: true,
            headline: "Agent loop · in flight",
            message: "Work · 42% — Goal → Plan → Tool → Verify.",
            active_count: 1,
            primary_session_id: "sess-mock-1",
            loop_chip: "Work",
            progress_pct: 42,
            loop_timeline_href: "/agents?session=sess-mock-1#agent-loop-timeline",
          },
          tool_outcome_strip: {
            enabled: true,
            headline: "Verify · tool outcomes",
            message: "1 session(s) need approve — review sim results and critic score before live.",
            pending_count: 1,
            primary_session_id: "sess-mock-2",
            tool_outcome_href: "/agents?session=sess-mock-2#tool-outcome-panel",
          },
          loop_guardrails_strip: {
            enabled: true,
            headline: "Closed loop · guardrails",
            message: "Max 5 turns · min 4.0/5 · $2.00 cap — 1 session(s) in flight.",
            max_turns: 5,
            min_score_label: "4.0/5",
            cost_cap_usd: 2,
            active_count: 1,
            guardrails_href: "/settings/harness#harness-closed-loop-presets",
            session_guardrails_href: "/agents?session=sess-mock-1#session-loop-guardrails",
          },
          goldmine_strip: {
            enabled: true,
            headline: "Goldmine · delta alerts",
            message: "Jobs Monitor · 3 new since last run — review before dispatch.",
            alert_count: 1,
            new_items_total: 3,
            primary_forager_name: "Jobs Monitor",
            primary_forager_id: "e2e-forager-goldmine-1",
          ...
          social_intel_strip: {
            ...
            message: "2 weak signal(s) this week — LOOP5 rubric score → Kanban task via Research Bee.",
            signal_count: 2,
            weekly_signal_count: 2,
            roadmap_refresh_due: false,
            loop5_preset_active: false,
            research_href: "/knowledge#research-bee",
            loop5_href: "/settings/harness#harness-closed-loop-presets",
            refresh_href: "/innovation-lab",
          },
          data_monitor_strip: {
            enabled: true,
            headline: "Data Monitor · wizard",
            message: "No data monitors yet — one sentence spawns a scheduled forager with extract schema.",
            monitor_count: 0,
            example_intent: "Track senior Python remote jobs in EU on public job boards",
          ...
          discovery_strip: {
            ...
            message: "Discover public URLs via Serper/Tavily, then bind feeds or channels to a forager.",
            keys_configured: true,
            tavily_configured: true,
          ...
          skill_factory_harness_strip: {
            enabled: true,
            headline: "Verified harness · Skill Factory",
            message: "No verified harness yet — Research → Queue → Library (Personal OS lite).",
            personal_os_lite: true,
            llm_ready: true,
            llm_smoke_ok: true,
            queue_actionable: 1,
            building_count: 0,
            ...
            gumroad_setup_hint: "Manual upload: exports/gumroad-upload/*.tar.gz + LISTING.md.",
            research_href: "/apps-tools/skill-factory#research",
            queue_href: "/apps-tools/skill-factory#queue",
            library_href: "/apps-tools/skill-factory#skill-factory-library",
            export_batch_href: "/apps-tools/skill-factory#export-batch",
            export_channels_href: "/apps-tools/skill-factory#export-channels",
            guide_href: "/apps-tools/skill-factory#guide",
          },
