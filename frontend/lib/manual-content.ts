...
export const APP_MANUAL_SECTIONS: ManualSection[] = [
  ...
  {
    id: "harness-four-cs",
    title: "Four Cs readiness audit (AI OS health)",
    paragraphs: [
      `Inspired by Nate Herk's **AI OS** framework: four dimensions that predict whether your agent stack runs reliably without babysitting.`,
      `**Context** — curated behavioral instructions (Settings → AI · harness), Wiki Layer hot tier, skill hot tier. Without context, every session re-explains your business.`,
      `**Connections** — MCP tools, supervisor routines (L3), routine webhooks (L4), Innovation Lab. Bees need pipes to the outside world — but simulate-first.`,
      `**Capabilities** — harness skills, Recipe Library, Pattern Router, Queen Maintainer (PR-only self-improvement). Capability without cadence = one-off demos.`,
      `**Cadence** — active routines, Knowledge Goals (L5), Maintainer daily budget. This is where automation compounds.`,
      `Live audit: [Settings → Harness → Rules overview](${MANUAL_HREFS.settingsHarness}#rules-overview). Read-only scores 0–100 — no auto-changes. Refresh weekly.`,
      `Env flags that raise scores: \`WIKI_LAYER_ENABLED\`, \`ROUTINES_ENABLED\`, \`RECIPES_ENABLED\`, \`QUEEN_MAINTAINER_ENABLED\`, \`SUPERVISOR_PATTERN_ROUTER_ENABLED\`.`,
    ],
    checklist: [
      { text: "Open Four Cs panel in Harness overview", href: MANUAL_HREFS.settingsHarness, linkLabel: "Harness settings" },
      { text: "Expand curated instructions if Context score is low", href: MANUAL_HREFS.settingsHarness, linkLabel: "Behavioral memory" },
      { text: "Schedule a verified recipe as routine (Cadence)", href: MANUAL_HREFS.knowledgeRecipes, linkLabel: "Recipes" },
      { text: "Connect MCP integrations if Connections is low", href: MANUAL_HREFS.integrationsHub, linkLabel: "Integrations hub" },
    ],
  },
  {
    id: "innovation-viability",
    title: "Innovation Lab viability gate",
    paragraphs: [
      `Before **Queen Maintainer** runs, every Innovation Lab proposal passes a **viability gate** — deterministic checks, no LLM. This blocks unsafe or underspecified handoffs.`,
      `**Required:** Innovation Lab enabled, Maintainer enabled, operator approval, implementation plan ≥80 chars, source prompt captured, **trust lane = simulate**, Maintainer daily budget available.`,
      `**High risk** proposals need operator checkbox \`acknowledge_high_risk\` — even then Maintainer is PR-only on \`queen-maintainer/*\` branches.`,
      `**Suggested paths** are validated against Maintainer allowlist — billing, \`.env*\`, prod compose, and secrets paths are blocked at draft time.`,
      `**Pre-tool denylist** scans plan + title for dangerous patterns: \`git push --force\`, \`deploy-prod.sh\`, \`docker compose down\`, \`rm -rf /\`, \`.env.prod\`, hardcoded secrets, \`curl | bash\`.`,
      `UI flow: Cockpit → Innovation or [Execution Studio → Innovation](${MANUAL_HREFS.integrationsStudioInnovation}). Each proposal shows inline viability checks. Use **Approve & queue** when all pass.`,
      `Never implemented: direct SSH to prod, writes to \`main\`, or skipping simulation for operator-facing outcomes.`,
    ],
    checklist: [
      { text: "Brainstorm with clear scope + modules", href: MANUAL_HREFS.integrationsStudioInnovation, linkLabel: "Innovation Lab" },
      { text: "Review viability banner before Approve & queue", href: MANUAL_HREFS.manualInnovationViability, linkLabel: "This section" },
      { text: "Check Agents → sessions for Maintainer PR", href: MANUAL_HREFS.agentsSessions, linkLabel: "Sessions" },
      { text: "Pre-tool safety rules reference", href: MANUAL_HREFS.manualMaintainerSafety, linkLabel: "Maintainer safety" },
    ],
  },
  {
    id: "maintainer-safety",
    title: "Queen Maintainer safety (PR-only + pre-tool denylist)",
    paragraphs: [
      `Queen Maintainer is the **only** automated coding agent in Queenswarm. It opens PRs — never merges to \`main\` directly. Operator merges after CI.`,
      `**Branch rule:** \`queen-maintainer/*\` only. Denylist at PR draft rejects changes touching billing, \`.env*\`, prod docker compose, secrets tables.`,
      `**Daily budget:** \`QUEEN_MAINTAINER_DAILY_RUN_LIMIT\` (default 3). Viability gate checks remaining runs before queue.`,
      `**Pre-tool denylist** runs on goal text before session spawn — same patterns as Innovation viability scan. Listed in Harness → Four Cs panel → Maintainer safety.`,
      `**Weekly routine** — optional cron Maintainer run for tech health; manual trigger via Settings → Harness → Operator loops → Maintainer webhook.`,
      `**Post-merge webhook** — after you merge a Maintainer PR, optional follow-up session for docs/tests (still PR-only).`,
      `Full operator instructions: \`docs/harness/QUEEN_MAINTAINER_INSTRUCTIONS.md\`. Audit: \`./scripts/operator-automation-ladder-audit.sh\` includes Maintainer env checks.`,
    ],
    checklist: [
      { text: "Enable QUEEN_MAINTAINER_ENABLED in prod env", href: MANUAL_HREFS.settingsHarness, linkLabel: "Harness loops" },
      { text: "Review pre-tool denylist in Four Cs panel", href: MANUAL_HREFS.manualHarnessFourCs, linkLabel: "Four Cs" },
      { text: "Innovation Lab → viability before queue", href: MANUAL_HREFS.manualInnovationViability, linkLabel: "Viability gate" },
      { text: "Merge PR only after CI green", href: MANUAL_HREFS.agentsSessions, linkLabel: "Sessions" },
    ],
  },
  {
    id: "agent-workflows",
