...
export type RolloutPhase = "phase0" | "phase1" | "phase2" | "phase3" | "phase4" | "phase5" | "phase6" | "phase7" | "ops";
...
export const ROLLOUT_PHASE_LABELS: Record<RolloutPhase, string> = {
  ...
  phase7: "Fáza 7 · Autonomous Agent OS + revenue swarms (Q2–Q4 2026)",
...
export const PLATFORM_ARCHITECTURE_LAYERS: ArchitectureLayer[] = [
  ...
  {
    id: "host",
    label: "Host · Cron automation",
    tone: "zinc",
    nodes: [
      { id: "cron-backup", label: "DB backup", detail: "Daily 02:00 UTC · pg_dump gzip · 7d retention" },
      { id: "cron-disk", label: "Disk retention", detail: "Daily cache >24h · monthly deep clean" },
      { id: "cron-watchdog", label: "Health watchdog", detail: "Every 15 min · auto-restart on /health fail" },
      { id: "cron-ssl", label: "SSL renew", detail: "Certbot 1st & 15th · nginx reload" },
    ],
  },
  {
    id: "llm",
...
export const LIVE_PLATFORM_CAPABILITIES: PlatformCapability[] = [
  ...
  },
  {
    ...
    howItWorks: "Admin panel agreguje system status + cross-tenant audit. Napojené na host cron watchdog.",
    ...
    stack: { backend: ["command_center_admin.py", "system_status.py"], frontend: ["command-center-settings-panel.tsx"] },
  },
  {
    id: "ops-db-backup",
    section: "Ops · Self-healing",
    name: "DB backup (daily cron)",
    status: "live",
    summary: "Postgres pg_dump → gzip, rotácia 7 dní, fail ak dump <1 KB.",
    howItWorks: "Host cron 02:00 UTC · scripts/backup_db.sh · container queenswarm_prod-postgres-1.",
    value: "Obnova DB bez manuálneho SSH — zálohy v /root/backups.",
    competitiveEdge: "Overená veľkosť dumpu — žiadne prázdne 0 B zálohy.",
    stack: { backend: ["scripts/backup_db.sh"] },
  },
  {
    id: "ops-disk-retention",
    section: "Ops · Self-healing",
    name: "Disk retention (daily + monthly)",
    status: "live",
    summary: "Docker build cache cleanup — denne >24h, mesačne >30 dní.",
    howItWorks: "scripts/audit-disk-cleanup.sh · nikdy nesiahne na queenswarm_prod kontajnery.",
    value: "Disk neprekročí 80 %+ bez zásahu operátora.",
    competitiveEdge: "Bezpečný prune vs. manuálne docker system prune na produkcii.",
    stack: { backend: ["scripts/audit-disk-cleanup.sh", "scripts/install-ops-automation-cron.sh"] },
  },
  {
    id: "ops-health-watchdog",
    section: "Ops · Self-healing",
    name: "Health watchdog (15 min)",
    status: "live",
    summary: "Edge /health smoke → auto-restart backend/frontend/celery pri zlyhaní.",
    howItWorks: "scripts/health-watchdog.sh volá health-check.sh, potom compose restart.",
    value: "Krátke výpadky sa samy zahoja bez nočného budenia.",
    competitiveEdge: "Watchdog na hoste + Command Center metriky v jednej appke.",
    stack: { backend: ["scripts/health-watchdog.sh", "scripts/health-check.sh"] },
  },
  {
    id: "ops-ssl-renew",
    section: "Ops · Self-healing",
    name: "SSL auto-renew",
    status: "live",
    summary: "Let's Encrypt certbot renew 1. a 15. deň v mesiaci + nginx reload.",
    howItWorks: "scripts/renew-letsencrypt.sh · webroot cez nginx ACME.",
    value: "TLS expirácia bez kalendárneho reminderu.",
    competitiveEdge: "Renew + reload v jednom cron kroku.",
    stack: { backend: ["scripts/renew-letsencrypt.sh", "scripts/issue-letsencrypt.sh"] },
  },
  {
    id: "ops-security-audit",
    section: "Ops · Self-healing",
    name: "Security exposure audit (monthly)",
    status: "live",
    summary: "Mesačný audit otvorených portov a nginx exposure na hoste.",
    howItWorks: "scripts/audit-host-exposure.sh · log /var/log/queenswarm-security-audit.log.",
    value: "Skoré varovanie pred zbytočne verejnými službami.",
    competitiveEdge: "Súčasť ops plánu — nie externý penetration test vendor.",
    stack: { backend: ["scripts/audit-host-exposure.sh"] },
  },
  {
    id: "ops-celery-retention",
    section: "Ops · Self-healing",
    name: "Celery beat retention & sweeps",
    status: "live",
    summary: "Agent stale sweep (2 min), tenant audit retention (týždenne), supervisor digest (hodinovo).",
    howItWorks: "Celery beat · AGENT_STALE_SWEEP + TENANT_AUDIT_RETENTION + SUPERVISOR_AUDIT_DIGEST env.",
    value: "DB a agent queue ostávajú zdravé bez manuálneho čistenia.",
    competitiveEdge: "App-level self-healing v rovnakej platforme ako agenti.",
    stack: { backend: ["beat_schedule.py", "agent_stale_sweep", "tenant_audit_retention_tick"] },
  },
  {
    id: "ops-slo-check",
    section: "Ops · Self-healing",
    name: "SLO edge check (weekly)",
    status: "live",
    summary: "Týždenný p95 latency + success rate report pre /health endpoints.",
    howItWorks: "scripts/slo-check.sh → reports/slo/*.md · cron pondelok 08:00 UTC.",
    value: "Trend výkonu edge vrstvy bez externého APM.",
    competitiveEdge: "Lightweight SLO v repozitári — nie Datadog povinnosť pre solo.",
    stack: { backend: ["scripts/slo-check.sh"] },
  },
  {
    id: "responsive-pwa",
    section: "Platform",
  ...
  },
  {
    ...
    status: "live",
    summary: "Verified-workflow badges on dashboard panel.",
  ...
  },
  {
    ...
    status: "live",
    summary: "Solo operator home at /agentic-os — compose layer over existing swarm (14 modules, no new bees).",
    howItWorks: "`compose_operator_cockpit_snapshot()` + `GET /operator/cockpit`; Innovation Lab approve → Queen Maintainer PR.",
    value: "One screen for now-actions, trust lane, hotline, teleport, regret, and immune posture.",
  ...
  {
    id: "operator-loop-live",
    section: "Settings",
    name: "Operator Loop — daily command center",
    status: "live",
    summary: "Unified snapshot: overnight dump, morning brief, publish queue, trading, prioritized actions.",
    howItWorks: "`GET /solo-operator/operator-loop` composes verified subsystems; Telegram 07:30 UTC optional.",
    value: "10-min morning workflow for solo operator — no tab hopping.",
    competitiveEdge: "Hermes-style morning OS bez monolitného agenta.",
    routes: ["/settings/harness"],
    stack: {
      backend: ["operator_loop.py", "operator_loop_tasks.py"],
      frontend: ["operator-loop-panel.tsx"],
    },
  },
  {
    id: "trading-cockpit-live",
    section: "Integrations",
    name: "Trading Cockpit (paper + Polymarket)",
    status: "live",
    summary: "Paper crypto simulation + Polymarket real-money prep checklist in Execution Studio.",
    howItWorks: "`GET /trading-cockpit` — lane config, funding, P&L, polymarket_readiness steps.",
    value: "Single panel for trading agent — deposit paper, tick, principles, live prep.",
    competitiveEdge: "Simulate-first prediction markets — Kalshi removed; Polymarket only.",
    routes: ["/integrations"],
    stack: {
      backend: ["trading_cockpit.py", "prediction_market_trading.py"],
      frontend: ["execution-studio-trading-cockpit-panel.tsx"],
    },
  },
  {
    id: "publish-performance-live",
    section: "Integrations",
    name: "Publish Performance Loop",
    status: "live",
    summary: "Aggregate publish audit → simulate rate, live posts, channel breakdown, insights.",
    howItWorks: "`GET /publish-performance` reads execution_studio recent_activity publish_* events.",
    value: "Close the loop: publish → measure → next action.",
    competitiveEdge: "Built-in analytics lane — not external social dashboard.",
    routes: ["/integrations"],
    stack: {
      backend: ["publish_performance.py"],
      frontend: ["execution-studio-publish-performance-panel.tsx"],
    },
  },
];
...
export const PLANNED_PLATFORM_CAPABILITIES: PlannedCapability[] = [
  {
    ...
    rationale: "✅ Automated via operator-launch-gate; only Hetzner email remains manual.",
  ...
  {
    id: "ops-host-automation",
    name: "Host ops automation (cron suite)",
    priority: "P0",
    impact: "high",
    rolloutPhase: "ops",
    owner: "operator",
    auditGate: "APPLY=1 ./scripts/install-ops-automation-cron.sh",
    summary: "✅ Shipped — DB backup, disk retention, health watchdog, SSL renew, security audit, weekly SLO.",
    rationale: "Solo operator self-healing — host crons + Celery retention bez SSH babysitting.",
    competitiveEdge: "Live v Atlas · Ops · Self-healing + Command Center metriky.",
    hints: "crontab marker queenswarm-ops-* · logrotate /etc/logrotate.d/queenswarm-ops",
    targetPhase: "Ops",
  },
  {
  ...
  {
    id: "selective-recall",
    name: "Selective HiveMind recall mode",
    priority: "P1",
    impact: "high",
    rolloutPhase: "phase4",
    week: 3,
    owner: "dev",
    auditGate: "./scripts/mission-phase4-selective-recall-audit.sh",
    summary: "Graph-neighbour RAG + similarity pruning + token budget cap for supervisor prompts.",
    rationale: "✅ Shipped — tenant recall settings + vault document lane on Knowledge hub.",
    competitiveEdge: "40–70% token savings target without external rerouter hacks.",
    targetPhase: "Knowledge",
  },
  {
    ...
    week: 4,
    owner: "dev",
    auditGate: "./scripts/mission-phase4-venice-mcp-audit.sh",
    summary: "One-click Venice connector preset + unified Tool Hub with cost/speed hints.",
    rationale: "✅ Shipped — venice_mcp Phase 3 template + UnifiedToolHubPanel on Integrations hub.",
  ...
  },
  {
    ...
    week: 5,
    owner: "dev",
    auditGate: "./scripts/mission-phase4-unified-savings-audit.sh",
    summary: "Merge time-saved + LLM cost saved vs quality baseline — „koľko si ušetril“.",
    rationale: "✅ Shipped — UnifiedSavingsPanel on /costs with merged headline value.",
    competitiveEdge: "Merateľný hive ROI, nie vanity chat metrics.",
    targetPhase: "Dashboard",
  },
  {
    id: "overnight-voice-report",
    name: "Voice Overnight Swarm Report",
    priority: "P2",
    impact: "medium",
    rolloutPhase: "phase4",
    week: 5,
    owner: "dev",
    auditGate: "./scripts/mission-phase4-overnight-voice-audit.sh",
    summary: "Ballroom TTS morning briefing from latest Dump & Sleep overnight report.",
    rationale: "✅ Shipped — Listen to briefing on dashboard + Ballroom with synthesize_speech.",
    competitiveEdge: "Wake-up wow moment vs static markdown-only briefings.",
    targetPhase: "Ballroom",
  },
  {
  ...
  },
  {
    ...
    week: 2,
    owner: "dev",
    auditGate: "./scripts/mission-phase5-pattern-explorer-audit.sh",
    summary: "Show which agentic patterns swarm used and why (from context_summary).",
    rationale: "✅ Shipped — PatternExplorerCard on dashboard + full catalog at /settings/harness.",
  ...
  {
    id: "cross-swarm-knowledge-transfer",
    name: "Cross-swarm knowledge transfer",
    priority: "P0",
    impact: "high",
    rolloutPhase: "phase7",
    week: 1,
    owner: "dev",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Pollen winners + verified recipes auto-suggested across swarms (trading → marketing).",
    rationale: "Hermes / multi-agent memory trend — HiveMind edges bez manuálneho copy.",
    competitiveEdge: "Graph-backed transfer vs isolated agent silos.",
    targetPhase: "Learning",
  },
  {
    id: "trading-swarm-template",
    name: "Trading Swarm Builder template",
    priority: "P0",
    impact: "high",
    rolloutPhase: "phase7",
    week: 1,
    owner: "dev",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Wizard: Forager → Analysis consensus → Risk → Executor — Polymarket lane.",
    rationale: "Cockpit existuje; chýba one-click swarm pre trading empire workflow.",
    competitiveEdge: "Verified multi-bee trading — nie single prompt bot.",
    targetPhase: "Swarms",
  },
  {
    id: "analysis-swarm-consensus",
    name: "Analysis Swarm (3-model consensus)",
    priority: "P0",
    impact: "high",
    rolloutPhase: "phase7",
    week: 2,
    owner: "dev",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Cheap multi-LLM consensus bee before trade/content decisions.",
    rationale: "X bookmark trend — narrow agents + consensus reduces hallucination risk.",
    competitiveEdge: "LiteLLM router native — simulate-first default.",
    targetPhase: "Supervisor",
  },
  {
    id: "trade-to-content-pipeline",
    name: "Trade → Content auto-generator",
    priority: "P1",
    impact: "high",
    rolloutPhase: "phase7",
    week: 3,
    owner: "dev",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Verified paper fill / simulate trade → publish pack draft → Publish Queue.",
    rationale: "Hybrid revenue: trading performance + faceless content — top combo from analysis.",
    competitiveEdge: "Single hive loop — not Zapier glue.",
    targetPhase: "Integrations",
  },
  {
    id: "trading-content-hybrid-swarm",
    name: "Trading + Content Hybrid swarm",
    priority: "P0",
    impact: "high",
    rolloutPhase: "phase7",
    week: 4,
    owner: "both",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Polymarket paper/live + auto YouTube/X/newsletter drafts from outcomes.",
    rationale: "Biggest out-of-box biznis play from X signals — dual revenue stream.",
    competitiveEdge: "Transparent verified loop + content factory.",
    targetPhase: "Product",
  },
  {
    id: "forager-intelligence-v2",
    name: "Forager Intelligence Loop v2",
    priority: "P1",
    impact: "high",
    rolloutPhase: "phase7",
    week: 5,
    owner: "dev",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Daily MCP manifest diff + stale skill detection + Queen Maintainer PR drafts.",
    rationale: "Self-evolving harness — skills/tools stay fresh without manual babysitting.",
    competitiveEdge: "PR-only self-maintenance + graph proposals.",
    targetPhase: "Foragers",
  },
  {
    id: "skill-marketplace-2",
    name: "Skill Marketplace 2.0 (verified + revenue share)",
    priority: "P1",
    impact: "high",
    rolloutPhase: "phase7",
    week: 6,
    owner: "both",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "UGC trading/marketing recipes with performance transparency + marketplace cut.",
    rationale: "Virálne swarms → revenue — levelsio / indie hacker playbook.",
    competitiveEdge: "Cosine + imitation + verified outcomes gate.",
    targetPhase: "Commercial",
  },
  {
    id: "public-paper-trading-transparency",
    name: "Public paper-trading transparency page",
    priority: "P1",
    impact: "medium",
    rolloutPhase: "phase7",
    week: 7,
    owner: "both",
    auditGate: "docs/CAPABILITIES_SYNTHESIS_MAY2026.md",
    summary: "Read-only P&L + fill history for brand (paper first, no secrets).",
    rationale: "Polymarket bot $794k narrative — trust via transparency.",
    competitiveEdge: "Live verified metrics — not screenshot marketing.",
    targetPhase: "Product",
  },
];
...
const ROLLOUT_PHASE_ORDER: RolloutPhase[] = [
  "phase0",
  "phase1",
  "phase2",
  "phase3",
