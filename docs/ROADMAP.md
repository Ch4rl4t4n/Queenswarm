...
| POS-G | Dead code deletion (commercial routes) — after 2 weeks stable | ✅ Shipped |
...
| G4 | Backend commercial router archival (feature-flag removal) | ✅ |
| G5 | Frontend route + service deletion (inventory-driven) | ✅ |
...
| MK11 | **Seed factory batch** — one-click research from pending MK6 vertical seeds | ✅ Shipped | `catalog_wave_seed_batch_service` · `POST /dashboard/catalog-wave/seed-batch` |
...
| Jun 2026 | Second-brain / Obsidian threads | B (SB1–4) | SB1 ✅ · SB2 ✅ · SB3 ✅ · SB4 ✅ |
...
| SB3 | Capture approve → auto wikilink in vault export | P1 | 2 d | ✅ Shipped | Pending inbox · approve API · captures/*.md export |
| SB4 | Wiki-layer hits in ⌘K mission search | P2 | 2–3 d | ✅ Shipped | `wiki_hits` in mission-search · palette UI |
...
| TR3 | **Rubric score** in session report pre-approve | P2 | 1 d | ✅ | `session_report_rubric_service`, `GET /agents/sessions/{id}/report-rubric`, Info report |
...
| LR3 | Worker crash → auto-resume + mission feed notify | P2 | 3 d | ✅ | `worker_crash_auto_resume_service`, Celery `hive.worker_crash_auto_resume_tick`, Redis lease |
...
| DG8 | **Goldmine → product** — monitor niche → Skill Factory seed | P2 | 3 d | ✅ | `forager_goldmine_factory_seed_service`, goldmine Factory seed button |
...
| DA10 | **Report critic closed loop** — rubric ≥4/5 before export (LOOP5 preset) | P1 | 1–2 d | ✅ | `analytics_report_critic_service.py` |
| DA11 | **Snapshot API** `GET /analytics-workspace/snapshot` | P0 | 1 d | ✅ | Single cached read |
| DA12 | **E2E + operator manual** — wizard → session → approve export | P1 | 2 d | ✅ | `OPERATOR_ANALYTICS_WORKSPACE_MANUAL.md` · `analytics-workspace-journey.spec.ts` |
